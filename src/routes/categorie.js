...
async function categorie(fastify) {
	...
	const { getAllCategorieSchema, getCategorieSchema, createCategorieSchema, updateCategorieSchema } =
		createSchemas(fastify);

	fastify.get("/categorie", { schema: getAllCategorieSchema }, async (request, reply) => {
		const categorie = await categorieController.getAllCategorie();
		reply.send(categorie);
	});

	fastify.get("/categorie/:id", { schema: getCategorieSchema }, async (request, reply) => {
	...
	fastify.delete("/categorie/:id", { preHandler: requireRole(["ADMIN"], client) }, async (request, reply) => {
